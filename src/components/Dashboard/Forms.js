...
import { UpdateSpeculation } from "../../config/SpeculationModal";
...
export function ModificationForm( { soiltype, flowrate, sid } )
{
    const [ response, setResponse ] = useState(false);
    const [ inputs, setInputs ] = useState( { soiltype : soiltype, flow : flowrate } );
    ...
    const handleForm = async ( event ) => {
        ...
        console.log( inputs )
        try {
            await UpdateSpeculation ( sid, inputs )
            setResponse(1)
        } catch (error) {
            setResponse(0)
        }
    ...
    <form className="bg-white pb-4 pt-3 px-2">
        ...
        { response ? ( <div className="mb-3 alert alert-success"> <span className="color-gray" > modification fait avec succes </span> </div> ) : null }  
         {/* <!--text input --> */}
        <div className="form-outline mb-3 d-flex flex-column"> 
            ...
            <input type="text" name="soiltype" value={ inputs.soiltype } className="form-control" placeholder="sol sableux" required onChange={ handleInput } /> 
        </div>
        {/* <!--text input --> */}
        <div className="form-outline mb-4 d-flex flex-column"> 
            ...
            <div class="input-group">
                <input type="text" name="flow" value={ inputs.flow } className="form-control" placeholder="200" required onChange={ handleInput } />
